#include "../include/ros_qt_demo/mylabel.h"

#include <QDebug>
MyLabel::MyLabel(QWidget* parent) : QLabel (parent) {
  // setPalette(QPalette(Qt::white));
  // resize(parent->width(), parent->height());
  qDebug()<<"mylabel"<<parent->width()<<endl;
  ...
  // map_px = QPixmap(":/images/map.jpg");
  // parent->setPixmap(map_px);
  // image->lower();
  // resize(width(), height());
...
void MyLabel::paintEvent(QPaintEvent *event) {
  ...
  // imgg = new QImage;
  // imgg->load(":/images/map.jpg"); //载入图片到img对象中
  QPainter painter(this);
  painter.save();
  // painter.drawImage(QPoint(0, 0), imgg);
  ...
  //                                padR);  //圆心2，半径1，焦点2
  // RadialGradient.setColorAt(0, QColor(0, 253, 253, 255));  //渐变
...
void MyLabel::mouseReleaseEvent(QMouseEvent* event) {
  ...
  tim->stop();
  mousePressed = false;
  emit keyNumchanged(MyLabel::stop);
  update();
}
...
void MyLabel::resizeEvent(QResizeEvent *event){
    // QPixmap px = map_px.scaled(event->size());
    // image->setPixmap(px);
    // image->resize(event->size());
    // qDebug()<<event->size()<<endl;
    // MyLabel::resizeEvent(event);
}

double MyLabel::Pointdis(int a, int b, int x, int y) {
  return sqrt((double)((x - a) * (x - a) + (y - b) * (y - b)));
}
int MyLabel::getKeyNum() {
  int x, y;
  int keynum;
