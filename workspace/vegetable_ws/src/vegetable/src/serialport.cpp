...
std::mutex cv_m;
bool received_autowork = false; // 记录是否接收到 autowork
            ...
            if (feedback.length() > 1) {
                try {
                    if (feedback == AUTOStateToString(AUTOState::telecontrol)) {
                        ...
                        received_autowork = false; // 处理 telecontrol 时清除 autowork 标志
                    } else if (feedback == AUTOStateToString(AUTOState::autowork)) {
                        if (!auto_mode.load() && !received_autowork) {
